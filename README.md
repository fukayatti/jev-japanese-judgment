...
**注意**: 下の数値は、学習に使った26,150件の中から抜き出した評価用サンプル（`train_val_split`）での値で、学習データと重なっている。学習に一度も使っていないデータでの評価ではないため、未知データでの精度としては過大な可能性が高い（量子化方式どうしの相対比較には使える）。未使用データでの再評価は今後の課題。

| 項目 | 値 |
| --- | --- |
| Accuracy（epoch1、学習データ内の評価用サンプル500件） | 93.8% |
| ECE（キャリブレーション誤差） | 1.14% |

## ローカルPCで動かす（GPU不要）

量子化方式ごとの精度（上記の評価用サンプルのうち200件。学習データと重なるため絶対値は過大、方式間の比較用。`scripts/ask.py` の `load()` と `scripts/gguf_pipeline.py` で測定）:
